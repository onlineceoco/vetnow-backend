...
io.on("connection", socket => {
  ...
  socket.on("joinroom", async roomId => {
    if (socket.user.role === "user") {
      const isChatExist = await Message.findOne({ user: socket.user._id });
      isChatExist && socket.emit("send_exist_messages_from_db", isChatExist);
    }
  ...
  socket.on("save_to_db", async messages => {
    if (socket.user.role === "user" && messages) {
      const isUserChatExist = await Message.findOneAndUpdate(
        { user: socket.user._id },
        { $set: { message: messages } },
        { new: true },
      );
      if (!isUserChatExist) {
        await Message.create({ user: socket.user._id, message: messages });
      }
    }
  });
